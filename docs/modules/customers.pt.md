...
Consulta e criação de clientes do X3. A entidade `CustomerEntity` estende os dados específicos de cliente com informação partilhada do parceiro de negócio subjacente (ver [Parceiros de negócio](business-partners.pt.md)) e as respetivas moradas.
...
| `addresses`              | Lista de moradas associadas ao cliente, resolvida via `DataLoader` ([Moradas](addresses.pt.md)) |
...
| `defaultAddress`         | Sim         | Morada por omissão do cliente ([`CreateAddressInput`](addresses.pt.md#createaddressinput)) |
...
mutation {
  createCustomer(
    input: {
      ...
      defaultAddress: { code: "SEDE", addressLine1: "Rua Principal, 100", city: "LISBOA", country: "PT" }
    }
  ) {
    customerCode
    customerName
