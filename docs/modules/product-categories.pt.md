...
Serviço interno auxiliar — **não expõe resolvers GraphQL próprios**. É consumido internamente na criação de um [produto](products.pt.md) (campo `productCategoryCode` de `CreateProductInput`), de onde são herdadas propriedades por omissão (unidades, níveis de imposto, grupos estatísticos) quando não indicadas explicitamente.
