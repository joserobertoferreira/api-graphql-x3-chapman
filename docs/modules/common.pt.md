...
Gere o ciclo de emissão das credenciais (`appKey` / `appSecret`) usadas na autenticação HMAC (ver [Autenticação](auth.pt.md)). As credenciais estão associadas a um login/password válidos de utilizador do X3 — o serviço valida essas credenciais contra a tabela de utilizadores do X3 (senha decifrada com o parâmetro `CRYPTSECRE` do X3) antes de gerar qualquer chave.
...
Ambas as operações estão marcadas com `@Public()` — não exigem assinatura HMAC prévia (fazem sentido para _provisionar_ a própria credencial), mas exigem login/password válidos do X3.
...
Esta query está protegida pelo `AdminGuard` (cabeçalho `X-Admin-Key`, ver [Autenticação](auth.pt.md#rota-administrativa)) em vez do fluxo HMAC normal, por se destinar a ferramentas internas de configuração.
