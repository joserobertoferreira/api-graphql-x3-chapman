...
import { FiscalYear, Period, Prisma, SalesOrderType, SiteGroups } from 'src/generated/prisma/client';
import {
  AnalyticalEntry,
...
export class CommonService {
  ...
  async getPurchaseOrderTypeSequenceNumber(): Promise<PurchaseSequenceNumber[]> {
    ...
    try {
      ...
      `;
      const results: PurchaseSequenceNumber[] = await this.prisma.$queryRaw(query);
  ...
  async getObjectInformation(object: string): Promise<X3ObjectInfo | null> {
    ...
    try {
      ...
      `;
      const results: X3ObjectInfo[] = await this.prisma.$queryRaw(query);
