...
import { BusinessPartner, Prisma, Site } from 'src/generated/prisma/client';
...
export class BusinessPartnerService {
  ...
  async isIntersiteTransaction(
    ...
    if (senderType === LocalMenus.BusinessPartnerType.CUSTOMER) {
      ...
      if ('customerCode' in senderInfo) {
        senderCode = senderInfo.customerCode;
      } else {
        throw new Error(' Invalid data structure for Customer');
      }
    } else {
      ...
      if ('supplierCode' in senderInfo) {
        ...
        throw new Error(' Invalid data structure for Supplier');
      }
    }
